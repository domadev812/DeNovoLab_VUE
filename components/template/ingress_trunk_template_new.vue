      ...
      <div class="row">
		<div class="wrapper single">
			...
			<ol class="breadcrumb">
				  ...
				  <li class="breadcrumb-item active">Ingress Trunk Template Editor</li>
			</ol>
			<h1 class="page-header">Ingress Trunk Template Editor</h1>
			<router-link to="/template/ingress_trunk_template" class="btn btn_white back"><span class="dnl_icon dnl_import"></span> Back </router-link>
			<div class="white_pad">
				<tabs class="vertical">
						...
						<section class="role_section">
							...
							<div class="col-md-4 col-sm-6">
								<div class="form-group">
									...
									<input type="text" name="template_name" v-model="template.template_name" class="form-control" placeholder="Template Name">
								</div>
							</div>
							<div class="clearfix"></div>
						</section>
						...
						<section class="role_section basic_section">
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									...
									<input type="text" name="ring_timer" v-model="template.ring_timer" class="form-control" placeholder="Enter Ring Timer">
								</div>
							</div>
                            <div class="col-md-4 col-sm-6">
								<div class="form-group fix-select">
					...
					<tab label="PASS" class="" icon="dnl_icon dnl_arrow-point-to-right">
						<section class="role_section">
							...
							<div class="col-md-4 col-sm-6">
								<div class="form-group fix-select">
									<label for="username">RPID:</label>
									<select class="selectable" v-model="template.is_rpid">
										<option v-for="item in pass_options" v-bind:value="item.id">
											{{ item.text }}
										</option>
									</select>
								</div>
							</div>
							<div class="col-md-4 col-sm-6" v-if="is_paid !== 'never'">
								<div class="form-group fix-select">
							...
							<div class="clearfix" v-if="is_paid !== 'never'"></div>
							<div class="col-md-4 col-sm-6" v-if="is_paid !== 'never'">
								<div class="form-group fix-select">
                            ...
                            <div class="col-md-4 col-sm-6">
								<label for="username">PAID:</label>
									<select class="selectable" v-model="template.is_paid" @change="is_paid = template.is_paid">
										<option v-for="item in pass_options" v-bind:value="item.id">
											{{ item.text }}
										</option>
									</select>
							...
							<div class="clearfix" v-if="is_paid !== 'never'"></div>
                            <div class="col-md-4 col-sm-6">
								<label for="username">OLI:</label>
								<select class="selectable" v-model="template.is_oli">
									<option v-for="item in pass_options" v-bind:value="item.id">
										{{ item.text }}
									</option>
								</select>
							...
							<div class="clearfix" v-if="is_paid == 'never'"></div>
                            <div class="col-md-4 col-sm-6">
								<label for="username">PCI:</label>
								<select class="selectable" v-model="template.is_pci">
									<option v-for="item in pass_options" v-bind:value="item.id">
										{{ item.text }}
									</option>
								</select>
                            ...
                            <div class="col-md-4 col-sm-6">
								<label for="username">PRIV:</label>
								<select class="selectable" v-model="template.is_priv">
									<option v-for="item in pass_options" v-bind:value="item.id">
										{{ item.text }}
									</option>
								</select>
							</div>
							<div class="clearfix little-space" v-if="is_paid !== 'never'"></div>
                            <div class="col-md-4 col-sm-6">
								<label for="username">DIV:</label>
								<select class="selectable" v-model="template.is_div">
									<option v-for="item in pass_options" v-bind:value="item.id">
										{{ item.text }}
									</option>
								</select>
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									<div class="checkbox checkbox-success">
										<label class="m-top-15" v-if="is_paid !== 'never'"></label>
				...
				<div class="button_set m-top-20">
					<a class="btn btn-primary mini" @click="validateForIngressTrunkTemplate">
						Save
					</a>
	...
	module.exports = {
		...
		data: function(){
            return {
				...
				pass_options: [
					{id: 'never', text: 'Never'},
					{id: 'pass_through', text: 'Pass Through'},
		...
		methods: {
			...
			organizeTemplateData(data) {
				...
				this.template = {
					...
					// temporary
					is_rpid: false,
					// is_rpid: data.rpid == 1? true: false,		// api have not rpid field
			...
			fetchTemplate() {
				const url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/' + this.id
				this.$http.get(url)
				.then(response => {
			...
			getIngressTrunkTemplateForRequest() {
				const ingress_trunk_template = Object.assign({}, this.ingress_trunk_template);

				ingress_trunk_template.profit_margin = 0;									// not exist in UI
				ingress_trunk_template.lnp_dipping_rate = 0;									// not exist in UI
				ingress_trunk_template.rpid_party = this.template.rpid_party;
				ingress_trunk_template.profit_type = "percentage";							// not exist in UI
				ingress_trunk_template.info = 0;												// not exist in UI
				ingress_trunk_template.ignore_ring = true;									// not exist in UI
				ingress_trunk_template.media_timeout = this.template.pdd_timeout;						// not sure
				ingress_trunk_template.rate_table_id = this.template.rate_table;
				ingress_trunk_template.billing_method = 0;									// not exist in UI
				ingress_trunk_template.bill_by = this.template.bill_by;
				ingress_trunk_template.amount_per_port = 0;									// not exist in UI
				ingress_trunk_template.random_table_id = 0;									// not exist in UI
				ingress_trunk_template.canada_other = 0;										// not exist in UI
				ingress_trunk_template.rpid_privacy = this.template.rpid_privacy;
				ingress_trunk_template.ignore_early_media = this.template.ignore_early_media;
				ingress_trunk_template.delay_bye_second = 0;									// not exist in UI
				ingress_trunk_template.rpid_screen = this.template.rpid_screen;
				ingress_trunk_template.trunk_type = "Ingress";								// not exist in UI
				ingress_trunk_template.rfc2833 = 0;											// not exist in UI
				ingress_trunk_template.inband = 0;											// not exist in UI
				ingress_trunk_template.res_strategy = this.template.multi_host;
				ingress_trunk_template.codecs_str = "string";								// not exist in UI
				ingress_trunk_template.re_invite_interval = 0;								// not exist in UI
				ingress_trunk_template.intl_route = 0;										// not exist in UI
				ingress_trunk_template.re_invite = 0;										// not exist in UI
				ingress_trunk_template.lnp_dipping = true;									// not exist in UI
				ingress_trunk_template.us_other = 0;											// not exist in UI
				ingress_trunk_template.display_name = this.template.is_display_name == true? 1: 0;
				ingress_trunk_template.rate_profile = this.template.rate_profile;
				ingress_trunk_template.ring_timeout = this.template.ring_timer;
				ingress_trunk_template.media_type = this.template.media_type;
				ingress_trunk_template.priv = this.template.is_priv == true? 1: 0;
				ingress_trunk_template.rate_rounding = this.template.rounding;
				ingress_trunk_template.wait_ringtime180 = 0;									// not exist in UI
				ingress_trunk_template.rpid_id_type = this.template.rpid_id_type;
				ingress_trunk_template.div = this.template.is_div == true? 1: 0;
				ingress_trunk_template.ignore_early_nosdp = this.template.ignore_early_nosdp;
				ingress_trunk_template.billing_rule = 0;										// not exist in UI
				ingress_trunk_template.rate_decimal = this.template.rounding_decimal_places;
				ingress_trunk_template.name = this.template.template_name;
				ingress_trunk_template.oli = this.template.is_oli == true? 1: 0;
				ingress_trunk_template.us_route = 0;											// not exist in UI
				ingress_trunk_template.pci = this.template.is_pci == true? 1: 0;
				ingress_trunk_template.paid = this.template.is_paid == true? 1: 0;
				// ingress_trunk_template.rpid = this.template.is_rpid == true? 1: 0;		// api rpid field not exist
				ingress_trunk_template.trunk_type2 = 0;										// not exist in UI
				ingress_trunk_template.canada_route = 0;										// not exist in UI
				ingress_trunk_template.max_duration = this.template.max_duration;

				return ingress_trunk_template;
			},
			createIngressTrunkTemplate() {
				const reqBody = this.getIngressTrunkTemplateForRequest();
				const url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/create';
				this.$http.post(url, reqBody)
					.then(response => {
						if (response.body.success) {
							this.setMessage('Ingress trunk template was created successfully');
							var thisvm = this;
							setTimeout(function(){
								thisvm.$router.push('/template/ingress_trunk_template');
							}, 3000);
						}
					})
					.catch(error => {
            ...
            validateForIngressTrunkTemplate() {
				this.$validator.validateAll()
				.then(this.createIngressTrunkTemplate)
				.catch(error => {
					console.log(error)
					this.setMessage({
