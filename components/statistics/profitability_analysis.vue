						...
						<table class="table table-striped table-hover carrier_groups centrum">
						  ...
						  <tbody>		
							...
							<tr v-for="origination in originations">
							  ...
							  <td></td>
							  <td>{{origination.ingress_cost}}</td>
							  <td>{{origination.egress_cost}}</td>
							</tr>
						  </tbody>
						</table>
					</div>
						...
						<table class="table table-striped table-hover carrier_groups centrum">
						  ...
						  <tbody>
							...
							<tr v-for="termination in terminations">
							  <td></td>
							  <td>{{termination.ingress_billed_time}}</td>
							  <td></td>
							  <td></td>
							  <td></td>
							  ...
							  <td></td>
							  <td>{{termination.ingress_cost}}</td>
							  <td>{{termination.egress_cost}}</td>
							</tr>
						  </tbody>
						</table>
					</div>
	...
	module.exports = {
		...
		data: function(){
            return {
				...
				loading: false,			
				orgination_search_field: ['ingress_billed_time', 'ingress_calls', 'non_zero_calls', 'ingress_cost', 'egress_cost'],
				termination_search_field: ['egress_billed_time', 'egress_calls', 'non_zero_calls', 'ingress_cost', 'egress_cost'],
				backupDatas: [],
				originations: [],
			    terminations: [],
				active_tab_index: 0,
		...
		methods: {
			...
			{								
				...
				var strURL;
				if(this.active_tab_index == 0)
				 	strURL = api.getReport_URL() + "?start_time=" + start_time + "&end_time=" + end_time + "&step=" + step + "&method=" + method + "&field=" + this.orgination_search_field[field_ind] + "&group=ingress_id";
				else
				{													
