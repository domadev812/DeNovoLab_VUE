				...
				<div class="select_group row">
					<div class="col-md-8 col-sm-10">
						<form class="form" @submit.prevent="validateBeforeSubmit">
							<input type="hidden" name="ftpid" v-model="email_id"/>
							<div class="row">
...
<script>
	...
	module.exports = {
		created () {
			...
			this.loading = true;
			this.$http.get(api.getEndpointUrl() + '/v1/config/mail_sender/' + id)
			.then(function(response) {
				if (response.body.success) {
					this.loading = false;
					console.log(response);
					var email_sender = response.body.payload;
					this.email_id = email_sender.id;

					this.sendername = email_sender.name;
					this.mail_server = email_sender.mail_server;
					this.mail_port = email_sender.mail_port;
					this.username = email_sender.username;
					this.pass = '*';
					this.authen = email_sender.authen ? 1 : 2
					// if(email_sender.authen == true) this.authen = 1;
					// else this.authen = 2;
					$("#ath").val(this.authen).trigger('change');
					this.secure = email_sender.secure;
					this.emailaddress = email_sender.email;
				}
			})
		...
		methods: {
			...
			validateBeforeSubmit () {
				...
				.then(() => {
					...
					// else authen = false;
					this.loading = true;
					this.$http.patch(api.getEndpointUrl() + '/v1/config/mail_sender/' + id, {
					...
					.then(function(response) {
						this.loading = false;
						this.setMessage('Email sender was edited successfully');
