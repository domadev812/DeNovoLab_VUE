			...
			<div class="white_pad configuration_pad">
				<div class="col-md-12 col-sm-12">
					<form class="form" @submit.prevent="validateBeforeSubmit">
						<input type="hidden" name="ftpid" v-model="ftpid"/>
							...
							<div class="row">
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<input type="text" name="alias" v-model="alias" class="form-control"  :class="{'input': true, 'is-danger': errors.has('alias') }" placeholder="Enter Alias" v-validate:alias="'required'">
								...
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<select class="selectable" v-model="file_breakdown">
											<option v-for="file_option in file_options" v-bind:value="file_option.id">
												{{ file_option.text }}
											</option>
										</select>
									</div>
								...
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<select class="selectable" v-model="include_headers">
											<option v-for="include_headers_option in include_headers_options" v-bind:value="include_headers_option.id">
												{{ include_headers_option.text }}
											</option>
										</select>
									</div>
								</div>
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<select class="selectable" v-model="compression">
											<option v-for="compression_option in compression_options" v-bind:value="compression_option.id">
								...
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<select class="selectable" v-model="frequency">
											<option v-for="frequency_option in frequency_options" v-bind:value="frequency_option.id">
												{{ frequency_option.text }}
											</option>
										</select>
									</div>
								</div>
								<div class="col-md-4 col-sm-6">
									<div class="form-group">
										...
										<select class="selectable" v-model="ftp_execute_on">
											<option disabled value="0" selected>Select Hour</option>
											<option v-for="hour in day_hours" v-bind:value="hour.id">
												{{ hour.text }}
											</option>
										</select>
										<!--<input type="text" name="ftp_execute_on" v-model="ftp_execute_on" class="form-control" :class="{'input': true, 'is-danger': errors.has('ftp_execute_on') }"  v-validate:ftp_execute_on="'required'" placeholder="Enter FTP Execute time" >
										<span v-show="errors.has('ftp_execute_on')" class="help is-danger">{{ errors.first('ftp_execute_on') }}</span>-->
									</div>
								</div>
							</div>
						  </section>
							...
							<div class="row">
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											...
											<select class="selectable" v-model="duration">
												<option v-for="duration_option in duration_options" v-bind:value="duration_option.id">
									...
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											...
											<!--<select2 :options="orc_options" v-model="trc" name="egress_release_cause" id="trcs">
											</select2>-->
							...
							<div class="row">
									...
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											...
											<input type="text" name="server_dir" v-model="server_dir" class="form-control"  :class="{'input': true, 'is-danger': errors.has('server_dir') }"  v-validate:alias="'required'" placeholder="Enter Server Directory">
	...
	module.exports = {
		created: function(){
			...
			this.$http.get(api.getEndpointUrl() + this.apiUrl + id,
			{
				...
				var ftp = response.body.payload;
				this.alias = ftp.name;
				this.frequency = ftp.frequency;
				this.ftp_execute_on = ftp.time;
				this.max_lines = ftp.max_line_per_file;
				
				if(ftp.include_header == true) this.include_headers = 1;
				else this.include_headers = 2;
				this.compression = ftp.compression;
				$("#freq").val(ftp.frequency).trigger('change');
				this.file_breakdown = ftp.file_breakdown;
				$("#filebreakdown").val(ftp.file_breakdown).trigger('change');
				$("#containheaders").val(this.include_headers).trigger('change');
				$("#filetype").val(ftp.file_type).trigger('change');
				if(ftp.non_zero == false) this.duration = 3;
				else this.duration = 2;
				$("#duration_").val(ftp.duration).trigger('change');
				$("#orcs").val(ftp.ingress_release_cause).trigger('change');
				$("#trcs").val(ftp.egress_release_cause).trigger('change');

				this.server_ip = ftp.ftp_server_ip;
				this.server_port = ftp.ftp_server_port;
				this.server_dir = ftp.ftp_directory;
				this.user_name = ftp.ftp_username;
				this.pass = ftp.ftp_password;
				this.orc = ftp.orig_return_code;
				this.trc = ftp.term_return_code;
				this.ftp_fields = ftp.include_fields;
				this.ots = ftp.ingress_trunks;
				this.tts = ftp.egress_trunks;
				this.ftpid = ftp.id;
			}, function(error) {
				this.loading = false;
				console.log(error);
		...
		data: function(){
			...
			//   ],
			//   trunk_options: [
			// 	  { key: 1, value: '1', label: 'Trunk 1' },
			// 	  { key: 2, value: '2', label: 'Trunk 2' },
			// 	  { key: 3, value: '3', label: 'Trunk 3' },
			// 	  { key: 4, value: '4', label: 'Trunk 4' },
			// 	  { key: 5, value: '5', label: 'Trunk 5' }
			//   ],
			  ...
			  file_breakdown: 0,
			  file_options:[
				  {id: 'as one big file', text: 'As one big file'},
				  {id: 'as hourly file', text: 'As hourly file'},
				  {id: 'as daily file', text: 'As daily file'},
			  ],
			  include_headers: 0,
		...
		methods: {
			...
			validateBeforeSubmit() {
				...
				this.$validator.validateAll().then(() => {
					console.log("validateBeforeSubmit1");
					...
					this.$http.patch(api.getEndpointUrl() + '/v1/config/ftp/' + id,
					{
						"active" : true,
						"name" : this.alias,
					...
					},
					{
						...
						function(response) {
							this.loading = false;
							console.log(response);
							this.setMessage('The FTP Configuration has been changed!');
							var thisvm = this;
							setTimeout(function(){
