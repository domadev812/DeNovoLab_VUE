      ...
      <div class="row">
		<div class="wrapper single">
			...
			<ol class="breadcrumb">
				  ...
				  <li class="breadcrumb-item active">Carrier Portal CDR Fields</li>
			</ol>
			<h1 class="page-header">Carrier Portal CDR Fields</h1>
...
<script>
	...
	module.exports = {
		...
		methods: {
			...
			undo: function() {
				// this.carrierFields = this.originalData.slice(0);
				// this.carrierFields = Object.assign([], this.originalData);
				this.carrierFields = JSON.parse( JSON.stringify( this.originalData ) );
			...
			submit: function() {
				...
				this.carrierFields.forEach(function(field, i){
					...
					temp.vendor_cdr_delivery = field.incoming_cdr;
					temp.client_cdr_delivery = field.outgoing_cdr;
					vm.push(temp);
				});
				console.log("fields:");
				console.log(vm);
				this.$http.patch(api.getEndpointUrl() + '/v1/config/cdr_visibility/edit',
				{
					"items": vm
				},
				{
					...
					function(response) {
						...
						this.$router.push('/configuration/carrier_portal_allowed_cdr_fields');
			...
			resetDefaults: function() {
				//this.carrierFields = this.defaultData.slice(0);
				// this.carrierFields = Object.assign([], this.defaultData);
		...
		created: function() {
				...
				items.forEach(function(item, i){
					...
					for (let f of self.carrierFields) {
						if (f.field_name == item.display_name) {
							f.id = item.id;
							f.incoming_cdr = item.vendor_cdr_delivery;
							f.outgoing_cdr = item.client_cdr_delivery;
						}
					}
				});
			...
			// this.originalData = this.carrierFields.slice(0);
			// this.originalData = Object.assign([], this.carrierFields);
