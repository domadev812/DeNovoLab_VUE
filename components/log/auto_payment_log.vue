		...
		<div class="row">
			<div class="wrapper single">
				...
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">Auto Payment Log</li>
				</ol>
				<h1 class="page-header">Auto Payment Log</h1>
				<div class="timeline">
						...
						<div class="col-md-12">
							<div class="table-responsive marged-30">
								<table class="table table-striped table-hover carrier_groups originators">
									<thead>
										<tr>
											<th v-if="showcolumns[0]">Client
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[1]">Submitted Time</th>
											<th v-if="showcolumns[2]">Response Time</th>
											<th v-if="showcolumns[3]">Method</th>
											<th v-if="showcolumns[4]">Charge Total</th>
											<th v-if="showcolumns[5]">Fee</th>
											<th v-if="showcolumns[6]">Status</th>
											<th v-if="showcolumns[7]">Paypal ID</th>
											<th v-if="showcolumns[8]">Transaction ID</th>
											<th v-if="showcolumns[9]">Card Number</th>
											<th v-if="showcolumns[10]">Card Expire Month</th>
											<th v-if="showcolumns[11]">Card Expire Year</th>
											<th v-if="showcolumns[12]">Return Code</th>
											<th v-if="showcolumns[13]">Error Information</th>
										</tr>
									</thead>
									<tbody>
										<tr v-if="!logs.length">
										...
										<tr v-for="log in logs">
											<td v-if="showcolumns[0]">{{log.client_name}}</td>
											<td v-if="showcolumns[1]">{{log.paid_on}}</td>
											<td v-if="showcolumns[2]">{{log.response}}</td>
											<td v-if="showcolumns[3]">{{log.type}}</td>
											<td v-if="showcolumns[4]">{{log.amount}}</td>
											<td v-if="showcolumns[5]">{{log.paypal_fee}}</td>
											<td v-if="showcolumns[6]">{{log.status}}</td>
											<td v-if="showcolumns[7]">{{log.paypal_id}}</td>
											<td v-if="showcolumns[8]">{{log.paypal_transaction_id}}</td>
											<td v-if="showcolumns[9]">{{log.cardnumber}}</td>
											<td v-if="showcolumns[10]">{{log.cardexpmonth}}</td>
											<td v-if="showcolumns[11]">{{log.cardexpyear}}</td>
											<td v-if="showcolumns[12]">{{log.return_code}}</td>
											<td v-if="showcolumns[13]">{{log.error}}</td>
										</tr>
									</tbody>
								</table>
							</div>
...
module.exports = {
	...
	data: function () {
		return {
			apiUrl: '/v1/finance/gateway_payments/',
			...
			log_table_columns: ['Client', 'Submitted Time', 'Response Time',
				'Method', 'Charge Total', 'Fee', 'Status', 'Paypal ID', 'Transaction ID', 
				'Card Number', 'Card Expire Month', 'Card Expire Year', 'Return Code', 'Error Information'],
	...
	methods: {
		handleChange() {
			...
			if (_.indexOf(this.log_table_columns, 'Submitted Time') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Response Time') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Charge Total') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Fee') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Paypal ID') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Card Number') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Card Expire Month') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Card Expire Year') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Return Code') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Error Information') != -1) {
