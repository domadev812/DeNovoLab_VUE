		...
		<div class="row">
			<div class="wrapper single">
				...
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">User Sign-On History</li>
				</ol>
				<h1 class="page-header">User Sign-On History</h1>
				<div class="timeline">
						...
						<div class="col-md-12">
							<div class="table-responsive marged-30">
								<table class="table table-striped table-hover carrier_groups aligned-center">
									<thead>
										<tr>
											<th v-if="showcolumns[0]">Login at</th>
											<th v-if="showcolumns[1]">User
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[2]">Host</th>
											<th v-if="showcolumns[3]">Agent</th>
											<th v-if="showcolumns[4]">Result</th>
										</tr>
									</thead>
									<tbody>
										...
										<tr v-for="log in filterBy(logs, search)">
											<td v-if="showcolumns[0]">{{log.create_time}}</td>
											<td v-if="showcolumns[1]">{{log.user_name}}</td>
											<td v-if="showcolumns[2]">{{log.host}}</td>
											<td v-if="showcolumns[3]">{{log.agent}}</td>
											<td v-if="showcolumns[4]">{{log.msg}}</td>
										</tr>
									</tbody>
								</table>
							</div>
...
module.exports = {
	...
	data: function () {
		return {
			...
			log_table_columns: ['Login at', 'User', 'Host',
				'Agent', 'Result'],
	...
	methods: {
		handleChange() {
			if (_.indexOf(this.log_table_columns, 'Login at') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Host') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Agent') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Result') != -1) {
