	...
	<div class="container-fluid">
		<div class="row">
			<div class="wrapper single">
				...
				<h1 class="page-header">Credit Log</h1>
				<div class="timeline">
						...
						<div class="col-md-12">
							<div class="table-responsive marged-30">
								<table class="table table-striped table-hover carrier_groups aligned-center">
									<thead>
										<tr>
											<th v-if="showcolumns[0]">Modified By</th>
											<th v-if="showcolumns[1]">Modified From</th>
											<th v-if="showcolumns[2]">Modified To</th>
											<th v-if="showcolumns[3]">Modified On</th>
											<th v-if="showcolumns[4]">Carrier Name</th>
										</tr>
									</thead>
									<tbody>
										<tr v-if="!logs.length">
										...
										<tr v-for="log in logs">
											<td v-if="showcolumns[0]">{{log.modified_by}}</td>
											<td v-if="showcolumns[1]">{{log.modified_from}}</td>
											<td v-if="showcolumns[2]">{{log.modified_to}}</td>
											<td v-if="showcolumns[3]">{{log.modified_on}}</td>
											<td v-if="showcolumns[4]">{{log.carrier_name}}</td>
										</tr>
									</tbody>
								</table>
							</div>
...
module.exports = {
	...
	data: function () {
		return {
			...
			log_table_columns: ['Modified By', 'Modified From', 'Modified To',
				'Modified On', 'Carrier Name'],
	...
	methods: {
		query() {
			...
			"&carrier_name=" + this.carrier + "&modified_on_lt=" + this.start_date + "&modified_on_gt=" + this.end_date + "&order_by=id&order_dir=desc",
				{
					headers: {
		...
		handleChange() {
			...
			if (_.indexOf(this.log_table_columns, 'Modified From') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Modified To') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Modified On') != -1) {
		...
		submit: function()
		{
			...
			if(this.selected_id == 0){
				...
				vm.loading = true;
				vm.resource = vm.$resource(api.getEndpointUrl() + '/v1/log/credit/'+vm.selected_id);
				vm.resource.delete({id:vm.selected_id}).then(function(response){
					...
					vm.setMessage('The credit log have been deleted!')
				});
			}
		},
	},
...
<style>
table.table.table-striped.table-hover.carrier_groups.aligned-center tr td {
	text-align: left;
}

table.table.table-striped.table-hover.carrier_groups.aligned-center tr td:last-child {
	text-align: center;
}
