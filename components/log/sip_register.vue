		...
		<div class="row">
			<div class="wrapper single">
				...
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">Sip Register Log</li>
				</ol>
				<h1 class="page-header">Sip Register Log</h1>
				<div class="timeline">
					<div class="chart_select_header grided">
						<div class="col-md-3 col-sm-3">
							<label>Status:</label>
							<select2 :options="options" v-model="status" class="inline-block">
						...
						<div class="col-md-3 col-sm-3">
							<div class="form-group">
								...
								<input type="text" name="username" v-model="username" class="form-control" placeholder="Enter User name">
							</div>
						</div>
								...
								<table class="table table-striped table-hover carrier_groups aligned-center">
									<thead>
										<tr>
											...
											<th v-if="showcolumns[2]">Carrier Name</th>
											<th v-if="showcolumns[3]">IP</th>
											<th v-if="showcolumns[4]">Port</th>
											<th v-if="showcolumns[5]">Status</th>
											<th v-if="showcolumns[6]">Expire</th>
											<th v-if="showcolumns[7]">Contact</th>
											<th v-if="showcolumns[8]">Uptime</th>
										</tr>
									</thead>
									<tbody>
										<tr v-if="!logs.length">
										...
										<tr v-for="log in logs">
											<td v-if="showcolumns[0]">{{log.username}}</td>
											<td v-if="showcolumns[1]">{{log.trunk_name}}</td>
											<td v-if="showcolumns[2]">{{log.carrier_name}}</td>
											<td v-if="showcolumns[3]">{{log.network_ip}}</td>
											<td v-if="showcolumns[4]">{{log.network_port}}</td>
											<td v-if="showcolumns[5]">{{log.status}}</td>
											<td v-if="showcolumns[6]">{{log.expires}}</td>
											<td v-if="showcolumns[7]">{{log.contact}}</td>
											<td v-if="showcolumns[8]">{{log.time}}</td>
										</tr>
									</tbody>
								</table>
							</div>
...
module.exports = {
	...
	data: function () {
		return {
			...
			options: [
				...
				{ id: 'un-register', value: 'un-register', text: 'un-register' },
				{ id: 'registered', value: 'registered', text: 'registered' },
				{ id: 'register failed', value: 'register failed', text: 'register failed' },
			],
			options1: [
				{ id: 1, text: '10' },
				{ id: 2, text: '20' },
			...
			log_table_columns: ['Username', 'Trunk Name', 'Carrier Name',
				'IP', 'Port', 'Status', 'Expire', 'Contact', 'Uptime'],
	...
	methods: {
		query() {
			...
			"&status=" + this.status + "&username=" + this.username + "&uptime_lt=" + this.time1 + "&uptime_gt=" + this.time2 + "&order_by=id&order_dir=desc",
				{
					headers: {
		...
		handleChange() {
			...
			if (_.indexOf(this.log_table_columns, 'IP') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Port') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Expire') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Contact') != -1) {
			...
			if (_.indexOf(this.log_table_columns, 'Uptime') != -1) {
