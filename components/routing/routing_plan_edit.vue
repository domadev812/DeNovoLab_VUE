	...
	<div class="container-fluid">
		<div class="row">
			<div class="wrapper single">
				...
				<h1 class="page-header">Edit Routing Plan</h1>
...
<script>   
	...
	module.exports = {
		created: function() {
			...
			this.$http.get(api.getEndpointUrl() + '/v1/route/plan/' + id,
				{
					headers: {
					...
					function(response) {
						this.loading = false;
						this.name = response.body.payload.name;
						this.is_virtual = response.body.payload.is_virtual;
					},
					function(error) {
						this.loading = false;
						console.log(error);
		...
		methods : {
			...
			validateBeforeSubmit(){
				this.$validator.validateAll().then(() => {
					...
					this.$http.patch(api.getEndpointUrl() + '/v1/route/plan/' + id,
					requestForm,
					{
						headers: {
							"X-Auth-Token": auth.getToken()
						...
						function(response) {
							...
							this.setMessage('The routing plan has been modified!');
						},
						function(response) {
							this.loading = false;
							console.log(response);
							...
							this.setMessage({
								message: "Editing routing plan is failed!",
								type: 'error'
							});
						}
					)
